    ...
    if isinstance(crets, pd.DataFrame):
        ...
        for c in tmp.columns:
            ...
            if fi != 0:
                ...
                tmp = pd.concat([pd.DataFrame({c: [1.0]}, index=[first]), tmp])
        crets = tmp
        if append:
            toadd = crets.index.shift(1)[-1]
            crets = pd.concat(
                [crets, pd.DataFrame(np.nan, columns=crets.columns, index=[toadd])]
            )
    else:
