...
class Trade(object):
    ...
    def __init__(self, tid, ts, qty, px, fees=0.0, **kwargs):
        self.tid = tid
    ...
    def split(self, amt):
        ...
        t1 = Trade(
            self.tid, self.ts, amt, self.px, fees=ratio * self.fees, **self.kwargs
        )
        t2 = Trade(
            self.tid,
            self.ts,
            self.qty - amt,
            self.px,
            fees=(1.0 - ratio) * self.fees,
            **self.kwargs
        )
        return [t1, t2]

    def __repr__(self):
        return "<%s(%s, qty=%s, px=%s, ts=%s)>" % (
