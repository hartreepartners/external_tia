...
class MockDataManager(object):
    def __init__(self):
        data = {"FLDA": [1, 2, 3], "FLDB": ["a", "b", "c"], "FLDC": [99.0, 98.0, 97.0]}
...
class TestDataManager(unittest.TestCase):
    ...
    def test_cache_sub(self):
        ...
        for i, (sid, fld) in enumerate(
            [("SID1", "FLDA"), ("SID2", "FLDB"), ("SID3", "FLDC")]
        ):
    ...
    def _do_historical_cache_test(self, storage):
        ...
        res = cdm.get_historical(
            ["SID1", "SID2", "SID3"], ["FLDA", "FLDB", "FLDC"], start, end
        )
        self.assertEqual(6, self.dm.access_cnt)
    ...
    def test_hdf_nan(self):
        df = pd.DataFrame(
            {"FLDA": np.array([1.0, np.nan], dtype=float)}, index=["SID1", "SID2"]
        )
        self.access_cnt = 0
